...
class AlertService:
    ...
    async def evaluate(self,user_id:int,reading:ElectricityReading)->list[AlertTransition]:
        try:
            transitions: list[AlertTransition] = []
            settings=await self._repo.get_settings(user_id,local_now())
            if settings.enabled:
                transition = await self._evaluate_value(user_id,reading,settings,AlertType.LOW_BALANCE,reading.remaining_money,settings.low_balance_enabled,settings.balance_warning_threshold,settings.balance_critical_threshold)
                ...
                transition = await self._evaluate_value(user_id,reading,settings,AlertType.LOW_REMAINING_DAYS,days,settings.low_remaining_days_enabled,settings.remaining_days_warning_threshold,settings.remaining_days_critical_threshold)
                if transition: transitions.append(transition)
            ...
            return []
    async def _evaluate_value(self,user_id:int,r:ElectricityReading,s:AlertSettings,k:AlertType,v:float|None,enabled:bool,w:float,c:float)->AlertTransition|None:
        ...
        if level is None:
            if active:
                active.status=AlertEventStatus.RESOLVED.value; active.resolved_at=active.last_seen_at=active.updated_at=now
                return AlertTransition(active, NotificationStage.RESOLVED)
            return
        ...
        if active:
            ...
            active.level,active.trigger_value,active.threshold_value=level.value,v,threshold; active.title,active.message,active.source_time,active.last_seen_at,active.updated_at=title,msg,parse_source_time(r.source_time),now,now
            if previous_level == AlertLevel.WARNING.value and level == AlertLevel.CRITICAL:
                return AlertTransition(active, NotificationStage.ESCALATED)
            return None
        event = await self._repo.create(user_id=user_id,area_id=r.area_id,room_id=r.room_id,building_name=r.building_name,floor_name=r.floor_name,room_name=r.room_name,alert_type=k.value,level=level.value,status=AlertEventStatus.ACTIVE.value,title=title,message=msg,trigger_value=v,threshold_value=threshold,source_time=parse_source_time(r.source_time),first_triggered_at=now,last_seen_at=now,resolved_at=None,created_at=now,updated_at=now)
        return AlertTransition(event, NotificationStage.ACTIVATED)
