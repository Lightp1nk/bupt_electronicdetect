...
import { clearCollectionSettings, getAlertSettings, getCollectionSettings, getNotificationBindings, runCollection, saveAlertSettings, saveCollectionSettings, saveNotificationBinding } from '@/api/electricity'
import type { AlertSettings, CollectionState, NotificationBinding } from '@/types/api'
...
const notification = ref<NotificationBinding | null>(null); const qqId = ref(''); const notificationEnabled = ref(true)
...
async function loadNotification(){ try { notification.value=(await getNotificationBindings()).find(x=>x.provider==='astrbot'&&x.platform==='qq')??null; qqId.value=notification.value?.target_id??''; notificationEnabled.value=notification.value?.enabled??true }catch{ notification.value=null } }
async function saveNotification(){ try { notification.value=await saveNotificationBinding({provider:'astrbot',platform:'qq',target_id:qqId.value,enabled:notificationEnabled.value}) }catch(cause){ error.value=cause instanceof ApiError?cause.message:'通知设置保存失败。' } }
...
<template>
  <section class="settings-page">
    ...
    <div class="settings-divider"></div><div class="collection-settings"><h2>通知设置</h2><label><input v-model="notificationEnabled" type="checkbox" />启用 AstrBot 通知</label><label>QQ 号<input v-model="qqId" inputmode="numeric" maxlength="20" placeholder="输入 QQ 号" /></label><p class="muted">{{ notification ? `已绑定 QQ：${notification.target_id}` : '未配置' }}</p><button class="text-button" @click="saveNotification">保存通知设置</button></div></section>
</template>
