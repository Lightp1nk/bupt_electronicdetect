...
async function saveNotification(){
  ...
  try {
    notification.value=await saveNotificationBinding({provider:'astrbot',platform:'qq',target_id:qqId.value,enabled:notificationEnabled.value})
    notificationStatus.value=await getNotificationStatus()
    notificationSaved.value = notificationEnabled.value ? '通知设置已保存。请使用同一 QQ 私聊机器人并发送 /电费绑定。' : '通知设置已保存，AstrBot 通知当前处于关闭状态。'
