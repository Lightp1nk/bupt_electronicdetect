...
async function loadNotification(){ try { notification.value=(await getNotificationBindings()).find(x=>x.provider==='astrbot'&&x.platform==='qq')??null; notificationStatus.value=await getNotificationStatus(); qqId.value=notification.value?.target_id??''; notificationEnabled.value=notification.value?.enabled??true }catch{ notification.value=null; notificationStatus.value=null } }
async function saveNotification(){ try { notification.value=await saveNotificationBinding({provider:'astrbot',platform:'qq',target_id:qqId.value,enabled:notificationEnabled.value}); notificationStatus.value=await getNotificationStatus() }catch(cause){ error.value=cause instanceof ApiError?cause.message:'通知设置保存失败。' } }
